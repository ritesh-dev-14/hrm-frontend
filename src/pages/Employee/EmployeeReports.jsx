  ...
  const fetchStats = async () => {
    try {
      ...
      if (res.data?.success) {
        ...
        data.pendingShoots =
          (data.totalShoots || 0) - (data.completedShoots || 0);
        setStats(data);
      } else {
    ...
    <div className="p-8 max-w-7xl mx-auto h-full overflow-y-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Real-time overview of your general project tasks and shoot employee
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 flex flex-col items-center text-center justify-between">
          <div className="w-full text-left">
            ...
            <p className="text-xs text-slate-500">
              General assigned workflow tasks.
            </p>
          </div>
          <div className="relative w-44 h-44 my-6">
            <svg
              viewBox="0 0 36 36"
              className="w-full h-full stroke-current text-indigo-100"
            >
              <path
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 flex flex-col items-center text-center justify-between">
          <div className="w-full text-left">
            <h2 className="text-lg font-bold text-slate-900">
              Shoot Tasks Completion
            </h2>
            <p className="text-xs text-slate-500">
              Shoot workspace deliverables & subtasks.
            </p>
          </div>
          <div className="relative w-44 h-44 my-6">
            <svg
              viewBox="0 0 36 36"
              className="w-full h-full stroke-current text-purple-100"
            >
              <path
                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                fill="none"
            ...
            <div className="absolute inset-0 flex items-center justify-center flex-col">
              <span className="text-3xl font-black text-purple-900">
                {stats.shootCompletionPercentage || 0}%
              </span>
              <span className="text-[10px] font-bold text-purple-500 uppercase tracking-widest mt-1">
                Shoot Tasks
              </span>
            </div>
          </div>
          <div className="w-full pt-4 border-t border-slate-100 flex justify-around text-xs font-semibold">
            <span className="text-purple-600 font-bold">
              {stats.completedShoots || 0} Approved
            </span>
            <span className="text-slate-400">|</span>
            <span className="text-slate-600">
              {stats.pendingShoots || 0} Pending
            </span>
          </div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {stats.shootSubtasks.map((st) => (
                  ...
                  >
                    ...
                    <td className="py-3 text-xs font-semibold text-slate-800">
                      {st.title}
                    </td>
                    <td className="py-3">
