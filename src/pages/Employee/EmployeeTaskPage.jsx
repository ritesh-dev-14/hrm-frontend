...
const EmployeeTaskPage = () => {
  ...
  const groupedTasks = useMemo(() => {
    ...
    filteredTasks.forEach(task => {
      const pName = task.taskItem?.task?.projectName || "General Project";
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 text-slate-800 p-6 md:p-10 font-sans selection:bg-indigo-100 relative overflow-hidden">
      ...
      {filteredTasks.length === 0 ? (
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="max-w-6xl mx-auto space-y-10 relative z-10">
          {Object.entries(groupedTasks).map(([projectName, projectTasks]) => (
            <div key={projectName} className="space-y-4">
              <div className="flex items-center gap-3 border-b border-slate-200/60 pb-2 mb-4">
                <Briefcase className="w-5 h-5 text-indigo-500" />
                <h2 className="text-xl font-bold text-slate-800">{projectName}</h2>
                <span className="bg-slate-100 text-slate-600 text-xs font-bold px-2 py-0.5 rounded-full">{projectTasks.length} {projectTasks.length === 1 ? 'task' : 'tasks'}</span>
              </div>
