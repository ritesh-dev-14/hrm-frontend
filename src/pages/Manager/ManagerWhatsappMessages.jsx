...
const ManagerWhatsappMessages = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch messages (Manager sees only their own)
      ...
      if (projects.length === 0) {
        ...
        // Filter to only manager's assigned projects
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Client Messages</h1>
              <p className="text-slate-600">View messages sent to your project clients</p>
            </div>
          </div>
          <motion.button
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6">
        <div className="flex gap-2 mb-4">
          ...
          {Object.values(filters).some((f) => f) && (
            <button
              onClick={() => setFilters({ projectId: "", datePreset: "", dateFrom: "", dateTo: "", status: "" })}
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Message Preview</th>
              ...
              {messages.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-slate-500">
                    No messages found. Messages will appear here once sent.
                  </td>
                </tr>
              ) : (
                messages.map((msg, idx) => (
                  <motion.tr key={msg.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.05 }} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600 truncate max-w-xs">{msg.messagePreview}</td>
      ...
      {showDetailModal && selectedMessage && <MessageDetailModal message={selectedMessage} onClose={() => setShowDetailModal(false)} />}
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1">Delivered At</label>
              <p className="text-slate-900 font-medium">{message.deliveredAt ? new Date(message.deliveredAt).toLocaleString() : "Pending..."}</p>
            </div>
            <div>
          ...
          <div>
            ...
            <div className="bg-slate-50 rounded-lg p-6 border border-slate-200 whitespace-pre-wrap text-sm text-slate-700 font-mono leading-relaxed max-h-96 overflow-y-auto">
              {message.fullContent}
            </div>
          </div>
        </div>
...
const SendMessageModal = ({ projects, selectedProject, onSelectProject, onSend, onClose, sending }) => {
  const socialMediaProjects = projects.filter(
    (p) =>
      (p.department?.name?.includes("Social") ||
        p.department?.name?.includes("Marketing") ||
        p.phone) && // Manager's assigned projects will have these
      p.phone // Must have phone number
  );
    ...
    >
      ...
      >
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 p-6 flex items-center justify-between">
          <h2 className="text-lg font-bold text-white">Send Client Message</h2>
          <button onClick={onClose} className="text-white hover:bg-indigo-500 p-2 rounded-lg transition-all">
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <div className="mb-4">
            ...
            {socialMediaProjects.length === 0 && (
              <p className="text-sm text-amber-600 mt-2">
                No projects available. Make sure you have projects assigned with client phone numbers.
              </p>
            )}
          </div>

          <p className="text-sm text-slate-600 mb-6">
            This will send today's upload status and social media statistics to the client via WhatsApp.
          </p>
...
export default ManagerWhatsappMessages;
