...
import ProfessionalLoader from "../../components/ProfessionalLoader.jsx";
  ...
  const handleDelete = useCallback(
    async (e, person) => {
      ...
      const confirmDelete = window.confirm(
        `Are you sure you want to permanently remove ${person.name || "this staff member"}?`,
      );
      ...
      try {
        ...
        if (res?.data?.success || res?.status === 200) {
          ...
          alert(
            "Server received the request but did not confirm data deletion.",
          );
        }
      } catch (err) {
        console.error("Deletion lifecycle failure:", err);
        const backendError =
          err?.response?.data?.message || "Internal database error.";
    ...
    <div className="min-h-screen bg-slate-50/50 text-slate-900 font-sans relative overflow-hidden pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 relative z-10">
        ...
        <motion.header
          ...
          className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 pb-6"
        >
          <div>
            ...
            <p className="text-sm text-slate-500 mt-2 font-medium max-w-xl">
              Manage corporate positioning, edit assignments, and structure
              organizational departments beautifully.
            </p>
          </div>
        ...
        <motion.main
          ...
          initial="hidden"
          animate="show"
          className="bg-white/60 backdrop-blur-xl rounded-[2rem] shadow-sm border border-slate-100/60 overflow-hidden flex flex-col min-h-[500px]"
        >
          {/* SEARCH & FILTERS ROW */}
          <div className="p-6 md:p-8 border-b border-slate-100/70 flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/50">
            ...
            <div className="flex items-center gap-1 bg-slate-100/50 p-1.5 rounded-xl border border-slate-200/50 w-full lg:w-auto overflow-x-auto custom-scrollbar">
              ...
              >
                Managers (
                {staff?.filter((p) => p.role === "MANAGER").length || 0})
              </button>
              <button
              ...
              >
                Employees (
                {staff?.filter((p) => p.role === "EMPLOYEE").length || 0})
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 bg-slate-50/30 overflow-x-auto">
            {loading ? (
              ...
              <div className="py-24 flex flex-col items-center justify-center text-center px-4">
                ...
                <p className="text-sm font-medium text-slate-500 max-w-sm mb-4">
                  {error}
                </p>
                <button
                  onClick={refresh}
              ...
              <div className="py-24 flex flex-col items-center justify-center text-center px-4">
                ...
                <p className="text-sm font-medium text-slate-500 max-w-sm">
                  Adjust your search or filter settings to see other team
                  members.
                </p>
              </div>
            ) : (
              <div className="min-w-[900px] w-full">
                ...
                <div className="grid grid-cols-12 gap-4 px-8 py-5 border-b border-slate-200 bg-slate-50/80 backdrop-blur-sm sticky top-0 z-10">
                  <div className="col-span-4 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    Staff Details
                  </div>
                  <div className="col-span-2 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    Reference ID
                  </div>
                  <div className="col-span-3 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    Department
                  </div>
                  <div className="col-span-2 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    Designation
                  </div>
                  <div className="col-span-1 text-[11px] font-bold uppercase tracking-wider text-slate-400 text-right">
                    Controls
                  </div>
                </div>
                  ...
                  {filteredStaff.map((person) => (
                    ...
                    >
                      ...
                      <div className="col-span-4 flex items-center gap-4 pr-4">
                        <div
                          className={`w-11 h-11 rounded-2xl font-black text-sm flex items-center justify-center shrink-0 border shadow-sm transition-transform group-hover:scale-105 ${
                            person.role === "MANAGER"
                      ...
                      <div className="col-span-2">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black tracking-wider uppercase border shadow-sm ${
                            person.role === "MANAGER"
                      ...
                      >
                        <button
                          onClick={(e) =>
                            handleAction(e, "EDIT_EMPLOYEE", person)
                          }
      ...
      />
      <HrViewDepartments
        isOpen={modal.type === "VIEW_DEPARTMENTS"}
        onClose={closeModal}
      />
      <HrEditEmployee
        isOpen={modal.type === "EDIT_EMPLOYEE"}
        employeeData={modal.data}
        onClose={closeModal}
        onSave={refresh}
      />
    </div>
