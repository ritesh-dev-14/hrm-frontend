...
// Keywords that identify the Social Media Department
...
const isSocialMediaProject = (project) => {
  const deptName = project?.department?.name?.toLowerCase()?.trim() ?? "";
  return SOCIAL_MEDIA_KEYWORDS.some((kw) => deptName.includes(kw));
};

const SocialMediaProjectsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 text-slate-900 font-sans relative overflow-hidden pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 relative z-10">
        ...
        >
          <div>
            <span className="text-xs font-bold tracking-widest uppercase text-cyan-600 mb-1 flex items-center gap-1.5">
              <Zap size={13} />
              Social Media Department
            </span>
          ...
          <motion.div
            ...
            className="flex items-center gap-3 bg-white border border-cyan-100 rounded-2xl px-5 py-3 shadow-sm"
          >
            <div className="w-10 h-10 bg-gradient-to-br from-cyan-500 to-indigo-500 rounded-xl flex items-center justify-center shadow-md shadow-cyan-500/30">
        ...
        >
          <div className="relative w-full group">
            ...
            <input
              ...
              className="w-full h-12 pl-11 pr-4 rounded-xl border border-slate-200 text-sm font-medium outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-500/10 transition-all bg-white"
            />
          </div>
        </motion.div>
          ...
          <div className="py-32 flex flex-col items-center justify-center bg-white/50 backdrop-blur-sm rounded-[2rem] border border-dashed border-slate-200 shadow-sm text-center">
            ...
            <p className="text-sm font-medium text-slate-500 max-w-sm">
              {searchQuery
                ...
                : "There are no projects assigned to the Social Media Department yet."}
            </p>
          </div>
        ) : (
          <motion.div
            ...
            <AnimatePresence>
              {filteredProjects.map((project) => (
                ...
                >
                  ...
                  {/* Dept badge */}
                  <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-cyan-600 bg-cyan-50 border border-cyan-100 px-2.5 py-1 rounded-lg mb-3 self-start">
                    <Zap size={10} />
                    {project.department?.name || "Social Media"}
                  </span>

                  {/* Title */}
                  <h3 className="text-xl font-black text-slate-900 mb-2 group-hover:text-cyan-600 transition-colors line-clamp-2">
                    {project.projectName}
                  </h3>
...
export default SocialMediaProjectsPage;
