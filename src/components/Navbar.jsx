...
const NAV_CONFIG = [
  ...
  { id: "marketing", label: "Marketing Reports", icon: TrendingUp, path: "/marketing", roles: ["ADMIN", "HR"] },
  { id: "marketing-monthly-reports", label: "Meeta Ads Calander", icon: BarChart2, path: "/marketing-monthly-reports", roles: ["ADMIN"] },
...
export default function ProfessionalSidebar({ children }) {
  ...
  useEffect(() => {
    ...
    socketInstance.on("today-upload-popup", (data) => {
      ...
      toast.info(
        <div>
          <strong>ðŸ“Œ Today's Upload Alert</strong><br />
          <span style={{ fontSize: 13 }}>{data.projectName} â€” {data.totalUploads} uploads</span>
        </div>,
        { autoClose: 6000 }
      );
    });
            ...
            return (
              <div key={item.id} className="flex flex-col">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-200 relative group outline-none ${active ? "text-indigo-50" : "text-slate-400 hover:text-slate-200 hover:bg-white/5"
                    }`}
                >
                  {active && (
                    <motion.div
                        ...
                        {item.children.map((child) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-[12px] transition-colors outline-none ${childActive
