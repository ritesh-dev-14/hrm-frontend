...
export default function ShootWorkspaceDetails() {
  ...
  const [selectedProjectForPrefill, setSelectedProjectForPrefill] =
    useState(null);
  ...
  // FIX: this previously had a stray non-ASCII character baked into the name
  // (`activeEditingSub尋taskId`), which meant `activeEditingSubtaskId` was
  // referenced everywhere else in the file but never actually declared —
  // that threw a ReferenceError the moment you tried to edit a subtask.
      ...
      try {
        ...
        setErrorMsg(
          err.response?.data?.message ||
            "Operational handshake timeout context block dropped.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleDeleteTask = async (e, taskId) => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to delete this planned shoot configuration?",
      )
    )
      return;
    ...
    try {
      ...
      setErrorMsg("");
      const res = await API.delete(
        `/api/shoot-workspaces/${shootId}/tasks/${taskId}`,
      );
      ...
      setErrorMsg(
        err.response?.data?.message ||
          "Could not delete planned shoot row node context.",
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleTaskSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditTaskMode) {
        res = await API.patch(
          `/api/shoot-workspaces/${shootId}/tasks/${activeEditingTaskId}`,
          payload,
        );
      } else {
      ...
      if (res.data?.success) {
        const refreshTasks = await API.get(
          `/api/shoot-workspaces/${shootId}/tasks`,
        );
      ...
      console.error(
        "Task mutations action processing failure mapping context:",
        err,
      );
      setErrorMsg(
        err.response?.data?.message ||
          "Remote database communication configuration logic exception.",
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleOpenTaskDetails = async (taskId) => {
    try {
      ...
      console.error(
        "Failed executing task details configuration compilation mapping layout:",
        err,
      );
      setErrorMsg(
        "Failed to resolve absolute target task configurations details pipeline.",
      );
    } finally {
  ...
  const handleOpenEditSubtaskModal = (sub) => {
    ...
    setSubtaskForm({
      ...
      referenceLinksRaw: sub.referenceLinks
        ? sub.referenceLinks.join(", ")
        : "",
      monthlySheetDayId: sub.dayId || sub.monthlySheetDayId || null,
    });
  ...
  const handleDeleteSubtask = async (subTaskId) => {
    if (!window.confirm("Are you sure you want to delete this shoot task?"))
      return;
    ...
    try {
      ...
      setErrorMsg("");
      const res = await API.delete(
        `/api/shoot-workspaces/${shootId}/tasks/${selectedTaskDetails.id}/subtasks/${subTaskId}`,
      );
      ...
      if (res.data?.success) {
        ...
        const refreshTasksIndex = await API.get(
          `/api/shoot-workspaces/${shootId}/tasks`,
        );
      ...
      console.error(
        "Failed processing target subtask deletion chain sequence:",
        err,
      );
      setErrorMsg(
        err.response?.data?.message ||
          "Subtask drop routine execution failure anomaly.",
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleAddSubtaskSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        videoType:
          subtaskForm.type === "REEL"
            ? subtaskForm.videoType || "HORIZONTAL"
            : null,
      ...
      if (res.data?.success) {
        const refreshSubtasks = await API.get(
          `/api/shoot-workspaces/${shootId}/tasks/${selectedTaskDetails.id}/subtasks`,
        );
      ...
      console.error(
        "Subtask deployment verification loop execution exception:",
        err.response || err,
      );
      setErrorMsg(
        err.response?.data?.message ||
          "Failed pushing active layout properties node into repository matrix.",
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleReviewSubtask = async (
    ...
    try {
      ...
      console.error(
        "Manager structural review modification pipeline failed:",
        err,
      );
      setErrorMsg(
        err.response?.data?.message ||
          "Failed processing submission evaluation response parameters.",
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleSelectProjectForPrefill = async (project) => {
    try {
      ...
      console.error(
        "Failed to load monthly sheets for project:",
        project.id,
        err,
      );
      setErrorMsg(
        err.response?.data?.message ||
          "Could not load monthly sheets for this project.",
      );
    } finally {
  ...
  const handleUpdateWorkspace = async () => {
    ...
    try {
      ...
      setErrorMsg(
        err.response?.data?.message || "Failed to save workspace mutations.",
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleDeleteWorkspace = async () => {
    try {
      ...
      if (res.data?.success && res.data.data?.deleted) {
        ...
        throw new Error(
          "Tear down operation structural response validation anomaly.",
        );
      }
    } catch (err) {
      ...
      setErrorMsg(
        err.response?.data?.message ||
          "Eviction aborted due to operational validation failure.",
      );
      setShowDeleteModal(false);
    } finally {
      setIsActionLoading(false);
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32 w-full bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-slate-500 text-sm mt-3 font-medium">
            Loading shoot workspace analytics...
          </p>
        </div>
      ) : errorMsg && !workspace ? (
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-slate-400 font-medium flex items-center gap-1 mt-0.5">
                          <Calendar size={12} /> Established on{" "}
                          {workspace?.createdAt
                    ...
                    <p className="text-slate-600 text-sm mt-4 max-w-3xl leading-relaxed bg-slate-50/70 p-3 rounded-xl border border-slate-100">
                      {workspace?.description ||
                        "No specific guidelines provided for this shoot brand track container."}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Production instructions and timeline configurations list.
                    Click a task row to view details & subtasks.
                  </p>
                </div>
                <button
              ...
              {tasks.length === 0 ? (
                <div className="text-center py-16 p-6">
                  ...
                  <h4 className="text-sm font-bold text-slate-700">
                    No Target Specifications Found
                  </h4>
                  <p className="text-xs text-slate-400 mt-0.5 max-w-sm mx-auto">
                    There are no operational tasks populated into this dynamic
                    campaign tracking index layout container framework yet.
                  </p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-sm">
                      {tasks.map((task) => {
                        const pendingCount =
                          task.pendingSubmissionsCount ??
                          (task.subtasks || []).filter(
                            (st) =>
                              st.status === "SUBMITTED" ||
                              st.status === "UNABLE_TO_SUBMIT",
                          ).length;
                          ...
                          >
                            <td className="py-4 px-5 max-w-xs">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="font-bold text-slate-900 block tracking-tight group-hover:text-indigo-600 transition">
                                  {task.title}
                                </span>
                                {pendingCount > 0 && (
                              ...
                              <span className="text-xs text-slate-400 line-clamp-1 mt-0.5">
                                {task.description ||
                                  "No specifications description provided."}
                              </span>
                              ...
                              {task.setupType ? (
                                <span
                                  className={`text-[10px] font-bold tracking-wider px-2 py-0.5 rounded-full ${
                                    task.setupType === "VERY_PREMIUM"
                                      ? "bg-amber-50 text-amber-700 border border-amber-100"
                                      : task.setupType === "PREMIUM"
                      ...
                      >
                        <div className="flex items-center gap-3 overflow-hidden pr-2">
                          ...
                          <div className="overflow-hidden">
                            ...
                            <span className="text-[11px] text-slate-400 font-medium block truncate font-mono mt-0.5">
                              {identifier} • {emp.position || "Staff Operator"}
                            </span>
                          </div>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-xl w-full overflow-hidden animate-scale-up flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button
                onClick={closeAndResetTaskModal}
                disabled={isActionLoading}
                className="p-1 hover:bg-slate-200 text-slate-400 hover:text-slate-600 rounded-lg transition disabled:opacity-50"
              >
                <X size={18} />
              ...
              <div className="p-5 overflow-y-auto space-y-4 flex-1 bg-white grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTaskForm({ ...taskForm, noOfPics: e.target.value })
                    }
                    className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-slate-50/40 disabled:opacity-50"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTaskForm({ ...taskForm, noOfReels: e.target.value })
                    }
                    className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-slate-50/40 disabled:opacity-50"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTaskForm({ ...taskForm, arrivalTime: e.target.value })
                    }
                    className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-slate-50/40 text-slate-600 disabled:opacity-50"
                  />
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setTaskForm({ ...taskForm, setupType: e.target.value })
                    }
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-md w-full p-6 animate-scale-up">
            <div className="flex items-center gap-3 text-rose-600 mb-3">
              ...
              <h3 className="text-lg font-bold text-slate-900">
                Confirm Shoot Deletion
              </h3>
            </div>

            <p className="text-slate-500 text-sm mb-6 leading-relaxed">
              Are you absolutely certain you want to tear down{" "}
              <strong>{workspace?.name}</strong>? This structural change is
              irreversible and will drop all associated production workflows and
              tasks indices.
            </p>
              ...
              {isSubtaskLoading ? (
                <div className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="text-xs text-slate-400 mt-2 font-medium">
                    Resolving task profile logs matrix...
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50/80 p-4 rounded-xl border border-slate-100 text-xs">
                    <div>
                      ...
                      <span className="text-slate-800 font-bold flex items-center gap-1 mt-0.5">
                        ...
                        {selectedTaskDetails?.noOfPics} Units
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium block">
                      ...
                      <span className="text-slate-800 font-bold flex items-center gap-1 mt-0.5">
                        <Video size={12} className="text-slate-400" />{" "}
                        {selectedTaskDetails?.noOfReels} Units
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium block">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed bg-slate-50/40 border border-slate-100 p-3 rounded-xl">
                      {selectedTaskDetails?.description ||
                        "No specific parameters detailed for this leaf configuration context."}
                    </p>
                  </div>
                  ...
                  <div className="border-t border-slate-100 pt-5">
                    ...
                    {subtasks.length === 0 ? (
                      <div className="text-center py-10 border border-dashed border-slate-200 rounded-xl bg-slate-50/30">
                        ...
                        <p className="text-xs text-slate-400 font-medium">
                          No active subtasks mapped across this execution node
                      ...
                      <div className="space-y-4">
                        {subtasks.map((sub) => {
                          ...
                          return (
                            <div
                              key={sub.id}
                              className="p-4 bg-white border border-slate-200 rounded-xl hover:border-indigo-100 shadow-sm transition flex flex-col justify-between gap-4 relative group"
                            >
                              ...
                              <div className="absolute top-4 right-4 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150 bg-white pl-2">
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleOpenEditSubtaskModal(sub)
                                  }
                                  disabled={isActionLoading}
                              ...
                              <div className="flex flex-col sm:flex-row justify-between items-start gap-3 pr-16">
                                <div className="space-y-1 flex-1">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    {(sub.dayId || sub.monthlySheetDayId) && (
                                  ...
                                  <p className="text-xs text-slate-500 leading-normal">
                                    {sub.description ||
                                      "No descriptions specified for this structural branch node item."}
                                  </p>
                                </div>
                                ...
                                {sub.submittedById && (
                                  <div className="flex items-center gap-1 bg-slate-50 px-2 py-0.5 border border-slate-200 rounded text-slate-600 font-semibold">
                                    ...
                                    <span className="font-mono text-[9px] text-slate-500">
                                      {sub.submittedById.substring(0, 8)}...
                                    </span>
                                    {sub.submittedAt && (
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-md w-full overflow-hidden animate-scale-up flex flex-col">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button
                onClick={closeAndResetSubtaskModal}
                disabled={isActionLoading}
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSubtaskForm({
                      ...subtaskForm,
                      referenceLinksRaw: e.target.value,
                    })
                  }
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-end gap-3">
                <button
                  ...
                  onClick={closeAndResetSubtaskModal}
                  disabled={isActionLoading}
                  className="px-3 py-1.5 font-semibold text-slate-600 hover:text-slate-800 transition disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
              ...
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
                  ...
                  {dayPickerStep === "project"
                    ? "Choose a Project"
                    : `Pick a Day — ${selectedProjectForPrefill?.projectName || ""}`}
                </h4>
              </div>
              <button
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-3">
              ...
              {dayPickerStep === "project" ? (
                ...
                <div className="text-center py-10">
                  <Info className="h-7 w-7 text-slate-300 mx-auto mb-2" />
                  <p className="text-xs text-slate-400 font-medium">
                    No monthly sheets found for this project.
                  </p>
                </div>
              ) : (
                monthlySheets.map((sheet) => (
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        setActiveSheetId(
                          activeSheetId === sheet.id ? null : sheet.id,
                        )
                      }
                    ...
                    >
                      <span>
                        {sheet.month}/{sheet.year} — {sheet.days?.length || 0}{" "}
                        day{sheet.days?.length === 1 ? "" : "s"}
                      </span>
                      ...
                      <div className="divide-y divide-slate-100">
                        {(sheet.days || []).length === 0 ? (
                          <p className="text-[11px] text-slate-400 italic px-3 py-3">
                            No days recorded on this sheet.
                          </p>
                        ) : (
                          sheet.days.map((day) => (
                            <button
                            ...
                            >
                              ...
                              <span className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">
                                ...
                                {" • "}
                                {day.reelType
                                  ? day.reelType.replace("_", " ")
                                  : day.postType
                                    ? day.postType.replace("_", " ")
                                    : "—"}
