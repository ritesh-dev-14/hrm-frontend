...
const ProjectDetailsView = ({ projectId, onBack, userRole = "" }) => {
  ...
  try {
    ...
    // SEO credentials
    if (isSeoDept) {
      if (editForm.seoName !== undefined) payload.seoName = editForm.seoName || null;
      if (editForm.seoContact !== undefined) payload.seoContact = editForm.seoContact || null;
      if (editForm.seoEmail !== undefined) payload.seoEmail = editForm.seoEmail || null;
      if (editForm.seoPassword !== undefined) payload.seoPassword = editForm.seoPassword || null;
    } else {
      // Social Media / Other credentials & details
      if (editForm.clientName !== undefined) payload.clientName = editForm.clientName || null;
      ...
      if (editForm.fbEmail !== undefined) payload.fbEmail = editForm.fbEmail || null;
      if (editForm.fbPassword !== undefined) payload.fbPassword = editForm.fbPassword || null;
      if (editForm.instaEmail !== undefined) payload.instaEmail = editForm.instaEmail || null;
      if (editForm.instaPassword !== undefined) payload.instaPassword = editForm.instaPassword || null;
      if (editForm.projectStartDate) payload.projectStartDate = new Date(editForm.projectStartDate).toISOString();
    }
  ...
  return (
    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-8 animate-fadeIn">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {/* DETAILS LAYOUT */}
        <div className={`grid grid-cols-1 ${isSeoDept ? '' : 'lg:grid-cols-3'} divide-y ${isSeoDept ? '' : 'lg:divide-y-0 lg:divide-x'} divide-slate-100`}>
          
          {/* MAIN COLUMN */}
          <div className={`${isSeoDept ? '' : 'lg:col-span-2'} p-8 space-y-8`}>
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-sm font-medium text-slate-800">{project.seoName || <span className="text-slate-400 italic text-xs">Not filled yet</span>}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-sm font-medium text-slate-800 flex items-center gap-1">
                        {project.seoContact ? (
                          <><Phone size={13} className="text-emerald-500" /> {project.seoContact}</>
                        ) : (
                          <span className="text-slate-400 italic text-xs">Not filled yet</span>
                        )}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">SEO Account Email</label>
                    {isEditing ? (
                      <input
                        type="email"
                        name="seoEmail"
                      ...
                      <p className="text-sm font-medium text-slate-800 break-all">
                        {project.seoEmail || <span className="text-slate-400 italic text-xs">Not filled yet</span>}
                      </p>
                    )}
                  </div>

                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1">SEO Account Password</label>
                    {isEditing ? (
                      <input
                        type="text"
                        name="seoPassword"
                        ...
                        <p className="text-sm font-medium text-slate-800 font-mono">
                          {project.seoPassword
                            ? showSeoPassword
                              ? project.seoPassword
                              : "••••••••"
                            : <span className="text-slate-400 italic text-xs not-italic font-sans">Not filled yet</span>}
                        </p>
                        {project.seoPassword && (
                          <button
                            type="button"
                            onClick={() => setShowSeoPassword(!showSeoPassword)}
                            className="text-slate-400 hover:text-slate-700 transition"
                          >
                            {showSeoPassword ? <EyeOff size={14} /> : <Eye size={14} />}
                          </button>
                        )}
                      </div>
                    )}
                ...
                {!isEditing && !project.seoEmail && isManager && (
                  <p className="text-xs text-emerald-700 bg-emerald-100 rounded-xl px-4 py-2.5 font-medium">
                    💡 Click <strong>Edit Project</strong> above to fill in your SEO account credentials.
                  </p>
                )}
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                {seoReportsLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 gap-3 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">
                    ...
                    <p className="text-xs text-slate-400">{isManager ? "Submit your first keyword ranking report." : "No SEO reports have been submitted."}</p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {report.remarks && (
                              <p className="mt-2 text-xs text-slate-500 italic">📝 {report.remarks}</p>
                            )}
                          </div>
                        </div>
                      </div>
