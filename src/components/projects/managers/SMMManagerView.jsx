...
const SMMManagerView = ({ projectId }) => {
  ...
  const padCalendarDays = (calendar) => {
    ...
    const formattedDays = days.map((day) => {
      return {
        ...
        contentCreativeSubmissionLinks:
          day.contentCreativeSubmissionLinks || [],
        creativeSubmissionLink: day.creativeSubmissionLink || null,
        submissionStatus: day.status || day.submissionStatus || null,
    ...
    return { ...calendar, days: formattedDays };
  };
  ...
  const fetchProjectDetails = async () => {
    try {
      ...
      if (err.response?.status === 403) {
        setError(
          "Access Denied: You are not assigned to this project. Please ask HR to assign you.",
        );
      } else {
        setError(
          err.response?.data?.message ||
    ...
    try {
      ...
      if (resData && resData.success) {
        ...
        try {
          ...
          fetch(
            "https://script.google.com/macros/s/AKfycbzXpksZ0y4Prof1D4KMHoR2n-5jTMvSIC5GOtqooR9j1EBqahZHrREDTx5AK1hXA708IA/exec",
            {
              method: "POST",
              mode: "no-cors",
              headers: {
            ...
            console.error("Google Sheet sync failed in background", err),
          );
        } catch (sheetErr) {
    ...
    try {
      ...
      const payload = {
        ...
        days: activeDaysPayload.map((day) => ({
          ...
          reelType:
            day.reelType && day.reelType !== "NONE" ? day.reelType : null,
          postType:
            day.postType && day.postType !== "NONE" ? day.postType : null,
          title: day.title || "",
  ...
  const handleUpdateExistingCalendar = async () => {
    ...
    try {
      ...
      const cleanedDays = activeDays.map((day) => {
        ...
        if (day.reelType && day.reelType !== "NONE")
          dayObj.reelType = day.reelType;
        if (day.postType && day.postType !== "NONE")
          dayObj.postType = day.postType;
      ...
      const payload = {
        days: cleanedDays,
        totalReels: selectedCalendar.totalReels
          ? parseInt(selectedCalendar.totalReels, 10)
          : 0,
        totalPosts: selectedCalendar.totalPosts
          ? parseInt(selectedCalendar.totalPosts, 10)
          : 0,
        totalReelsUploaded: selectedCalendar.totalReelsUploaded
          ? parseInt(selectedCalendar.totalReelsUploaded, 10)
          : 0,
        totalPostsUploaded: selectedCalendar.totalPostsUploaded
          ? parseInt(selectedCalendar.totalPostsUploaded, 10)
          : 0,
      };
      if (
        selectedCalendar.moodBoardLink &&
        selectedCalendar.moodBoardLink.trim() !== ""
      ) {
    ...
    <div className="min-h-screen bg-slate-50/50 text-slate-900 font-sans relative overflow-hidden pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 relative z-10">
        ...
        <motion.div
          ...
          className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 mb-2"
        >
          <div>
            ...
            {tasteItems.length > 1 && (
              <div className="mt-4 space-y-2">
                {tasteItems.slice(1).map((item, index) => (
                  <p
                    key={`taste-line-${index}`}
                    className="text-sm md:text-base font-semibold text-slate-700 break-words"
                  >
                    {item}
                  </p>
                ))}
                ...
                >
                  {project?.logo ? (
                    ...
                    <div className="flex flex-col items-center">
                      <svg
                        className="w-8 h-8 text-slate-300 mb-2 group-hover:text-indigo-400 transition-colors"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white/80 backdrop-blur-2xl rounded-[2rem] shadow-sm border border-slate-100/60 p-8 md:p-10"
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  disabled={!isEditingCredentials}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none text-sm font-medium text-slate-700 disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed"
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {formData.reference
                          ...
                          .map((ref, idx) => (
                            <div
                              key={idx}
                              className="group relative flex items-center gap-2 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg text-sm font-medium border border-indigo-100 cursor-pointer"
                            >
                              <span className="break-all line-clamp-1 max-w-[200px]">
                                {ref}
                              </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      disabled={!isEditingCredentials}
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-200 bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none text-sm disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed"
                ...
                <div className="space-y-4">
                  ...
                  {renderPasswordField(
                    "Instagram Password",
                    "instaPassword",
                    formData.instaPassword,
                    showInstaPassword,
                    setShowInstaPassword,
                    "••••••••••••",
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      disabled={!isEditingCredentials}
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-200 bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none text-sm disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed"
                      placeholder="youtube@channel.com"
                    />
                  </div>
                  {renderPasswordField(
                    "YouTube Password",
                    "youtubePassword",
                    formData.youtubePassword,
                    showYoutubePassword,
                    setShowYoutubePassword,
                    "••••••••••••",
                  )}
                  ...
                  <div className="space-y-2 mt-4 pt-4 border-t border-slate-200">
                    ...
                    <input
                      ...
                      disabled={!isEditingCredentials}
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-200 bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none text-sm disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed"
                      placeholder="linkedin@company.com"
                    />
                  </div>
                  {renderPasswordField(
                    "LinkedIn Password",
                    "linkedinPassword",
                    formData.linkedinPassword,
                ...
                <div className="space-y-4 md:col-span-2 lg:col-span-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-200">
                    ...
                    {renderPasswordField(
                      "Twitter / X Password",
                      "twitterPassword",
                      formData.twitterPassword,
                      showTwitterPassword,
                      setShowTwitterPassword,
                      "••••••••••••",
                    )}
                  </div>
                </div>
        ...
        >
          <h3 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-3">
            ...
            <span className="ml-auto text-xs font-bold bg-amber-100 text-amber-700 px-3 py-1 rounded-full border border-amber-200">
              ...
              {(project?.reasons || []).length === 1 ? "entry" : "entries"}
            </span>
          </h3>
          ...
          {!project?.reasons || project.reasons.length === 0 ? (
            <div className="text-center py-10 bg-amber-50/40 rounded-2xl border border-dashed border-amber-200">
              <svg
                className="w-10 h-10 text-amber-200 mx-auto mb-3"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-xs text-amber-400 mt-1">
                Reasons added from the Social Media projects page will appear
                here.
              </p>
            </div>
          ) : (
        ...
        >
          ...
          {sheetsLoading ? (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50/80 border-b border-slate-100/60">
                      ...
                      <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap text-right">
                        Actions
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100/60">
                    {monthlySheets.map((sheet) => {
                      const isThisCalendarLoading =
                        loadingCalendarId === sheet.id;
                      return (
                        <tr
                          key={sheet.id}
                          className={`group transition-colors hover:bg-slate-50/50 ${selectedCalendar?.id === sheet.id ? "bg-indigo-50/30" : ""}`}
                        >
                          <td className="px-6 py-5 whitespace-nowrap">
                            <p className="text-sm font-bold text-slate-900">
                              {new Date(0, sheet.month - 1).toLocaleString(
                                undefined,
                                { month: "long" },
                              )}{" "}
                              {sheet.year}
                          ...
                          <td className="px-6 py-5 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              <span className="flex items-center gap-1.5 text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-1 rounded-md">
                                🚀 {sheet.totalReelsUploaded} R
                              </span>
                              <span className="flex items-center gap-1.5 text-xs font-bold text-violet-700 bg-violet-50 border border-violet-100 px-2 py-1 rounded-md">
                          ...
                          <td className="px-6 py-5 whitespace-nowrap">
                            <div className="flex items-center gap-4">
                              {sheet.projectLogo || project?.logo ? (
                                <img
                                  src={sheet.projectLogo || project.logo}
                                  alt="Project logo"
                              ...
                              {sheet.moodBoardLink ? (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-sm font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2"
                                >
                                  Blueprint{" "}
                                  <svg
                                    className="w-3.5 h-3.5"
            ...
            <div className="bg-white/80 backdrop-blur-2xl rounded-[2rem] shadow-sm border border-slate-100/60 p-12 text-center">
              ...
              <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">
                No monthly strategy planning manifests initialized yet.
              </p>
            </div>
          )}
        </motion.div>
        ...
        {typeof document !== "undefined" &&
          createPortal(
            <AnimatePresence>
              {selectedCalendar && (
                <div
                  key="calendar-workspace-portal"
                  className="fixed inset-0 z-[99999] bg-slate-950/80 backdrop-blur-2xl flex flex-col p-2 sm:p-4 md:p-6 overflow-hidden"
                >
                  <motion.div
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-semibold text-slate-500">
                            Edit scripts, titles, frame descriptions, and
                            reference links in expanded layout
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setSelectedCalendar((prev) => ({
                                ...prev,
                                totalReelsUploaded: e.target.value,
                              }))
                            }
                            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm font-bold text-slate-800 shadow-xs"
                            placeholder="e.g. 5"
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setSelectedCalendar((prev) => ({
                                ...prev,
                                totalPostsUploaded: e.target.value,
                              }))
                            }
                            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm font-bold text-slate-800 shadow-xs"
                            placeholder="e.g. 3"
                      ...
                      <div className="flex flex-col flex-1 min-h-[400px]">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest">
                            Daily Tactical Matrix Breakdown
                          </h4>
                          <button
                            type="button"
                            onClick={() => {
                              setNewDayForm({ date: "", ...EMPTY_DAY_SHAPE });
                              setIsEditAddDayModalOpen(true);
                            }}
                            className="px-4 py-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-xs font-bold transition-colors shadow-sm"
                          >
                            + Add Row
                          </button>
                        </div>
                            ...
                            <table className="w-full text-left border-collapse">
                              <thead>
                                <tr className="bg-slate-100/95 sticky top-0 z-20 border-b border-slate-200 shadow-xs backdrop-blur-md">
                                  ...
                                  <th className="px-4 py-3.5 text-[11px] font-black text-slate-600 uppercase tracking-widest whitespace-nowrap">
                                    Reel Type
                                  </th>
                                  <th className="px-4 py-3.5 text-[11px] font-black text-slate-600 uppercase tracking-widest whitespace-nowrap">
                                    Post Type
                                    ...
                                    >
                                      ...
                                      <td className="px-4 py-3 whitespace-nowrap pt-4">
                                        ...
                                        >
                                          ...
                                          <option value="SHOOT_REQUIRED">
                                            SHOOT REQUIRED
                                          </option>
                                          <option value="AI_REQUIRED">
                                            AI REQUIRED
                                          </option>
                                          <option value="DATA_AVAILABLE">
                                            DATA AVAILABLE
                                          </option>
                                        </select>
                                      </td>
                                      ...
                                      <td className="px-4 py-3 min-w-[180px] pt-4">
                                        <div className="flex flex-col gap-2">
                                          {dayItem.submissionLinks?.length >
                                            0 &&
                                            dayItem.submissionLinks.map(
                                              (link, i) => (
                                                <a
                                                  key={`shoot-${i}`}
                                                  href={link}
                                                  target="_blank"
                                                  rel="noreferrer"
                                                  className="text-xs font-bold text-indigo-600 hover:text-indigo-800 break-all"
                                                >
                                                  Shoot Asset {i + 1} 🔗
                                                </a>
                                              ),
                                          ...
                                          {dayItem
                                            .contentCreativeSubmissionLinks
                                            ?.length > 0 &&
                                            dayItem.contentCreativeSubmissionLinks.map(
                                              (link, i) => (
                                                <a
                                                  key={`creative-arr-${i}`}
                                                  href={link}
                                                  target="_blank"
                                                  rel="noreferrer"
                                                  className="text-xs font-bold text-emerald-600 hover:text-emerald-800 break-all"
                                                >
                                                  Creative Asset {i + 1} 🔗
                                                </a>
                                              ),
                                          ...
                                          {dayItem.creativeSubmissionLink && (
                                            <a
                                              ...
                                              target="_blank"
                                              rel="noreferrer"
                                              className="text-xs font-bold text-fuchsia-600 hover:text-fuchsia-800 break-all"
                                            >
                                              Design Asset 🔗
                                            </a>
                                          )}
                                      ...
                                      <td className="px-4 py-3 whitespace-nowrap text-xs text-slate-500 pt-4">
                                        {formatDateTime(dayItem.reviewedAt)}
                                      </td>
                                      <td className="px-4 py-3 min-w-[150px] text-xs text-red-600 font-medium pt-4">
                    ...
                    {/* ADD DAY MODAL PORTAL (INSIDE EDIT VIEW) */}
                    {isEditAddDayModalOpen && (
                      <div className="fixed inset-0 z-[100000] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                        <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-slate-200">
                          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/90 backdrop-blur-md">
                            ...
                            <button
                              type="button"
                              onClick={() => setIsEditAddDayModalOpen(false)}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div className="space-y-1">
                                ...
                                <textarea
                                  rows={2}
                                  value={arrayToString(
                                    newDayForm.referenceLinks,
                                  )}
                                  onChange={(e) =>
                                    setNewDayForm({
                                      ...newDayForm,
                                      referenceLinks: stringToArray(
                                        e.target.value,
                                      ),
                                    })
                                  }
                              ...
                              <div className="space-y-1">
                                ...
                                <textarea
                                  rows={2}
                                  value={arrayToString(
                                    newDayForm.contentUploadLinks,
                                  )}
                                  onChange={(e) =>
                                    setNewDayForm({
                                      ...newDayForm,
                                      contentUploadLinks: stringToArray(
                                        e.target.value,
                                      ),
                                    })
                                  }
                              ...
                              <div className="space-y-1">
                                ...
                                <textarea
                                  rows={2}
                                  value={arrayToString(
                                    newDayForm.videoUploadLinks,
                                  )}
                                  onChange={(e) =>
                                    setNewDayForm({
                                      ...newDayForm,
                                      videoUploadLinks: stringToArray(
                                        e.target.value,
                                      ),
                                    })
                                  }
                          ...
                          <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (!newDayForm.date) {
                                  alert(
                                    "Please select a valid date for this row.",
                                  );
                                  return;
                                }
                                ...
                                setIsEditAddDayModalOpen(false);
                              }}
                              className="px-6 py-2.5 rounded-xl font-bold text-sm bg-emerald-600 hover:bg-emerald-700 text-white shadow-md transition-all"
                            >
                              Save Row
                            </button>
                          </div>
          ...
          {project?.assignments?.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {project.assignments.map((assignment) => (
                <div
                  key={assignment.id}
                  className="bg-white/60 backdrop-blur-xl border border-slate-200/60 p-4 rounded-2xl flex items-center gap-4 hover:shadow-md transition-shadow"
                >
            ...
            <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-6 text-center">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
                No other management stakeholders assigned.
              </p>
            </div>
          )}
        </motion.div>
        ...
        {typeof document !== "undefined" &&
          createPortal(
            <AnimatePresence>
              {isDrawerOpen && (
                <div
                  key="create-calendar-modal-portal"
                  className="fixed inset-0 z-[99999] bg-slate-950/80 backdrop-blur-2xl flex flex-col p-2 sm:p-4 md:p-6 overflow-hidden"
                >
                  <motion.div
                    initial={{ opacity: 0, scale: 0.97, y: 15 }}
                  ...
                  >
                    ...
                    <form
                      ...
                      className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-slate-100/50 flex flex-col"
                    >
...
const styles = {
  ...
  statusBadge: (status) => {
    const map = {
      PENDING: { bg: "#fef9c3", col: "#854d0e" },
      SUBMITTED: { bg: "#e0f2fe", col: "#0369a1" },
      APPROVED: { bg: "#dcfce7", col: "#166534" },
