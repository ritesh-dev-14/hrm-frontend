...
//         // Dynamic re-sync anchor: Keep the currently active view synchronized on updates
//         if (selectedCalendar) {
//           const freshSheet = (resData.data || []).find(s => s.id === selectedCalendar.id);
//           if (freshSheet) {
//             setSelectedCalendar(padCalendarDays(freshSheet));
//           }
//         }
//       }
//     } catch (err) {
...
//           {project?.logoUrl ? (
//             <img src={project.logoUrl} alt="Client logo" style={styles.logoImageLarge} />
//           ) : (
...
//           >
//             {isUploadingLogo ? "Uploading..." : project?.logoUrl ? "Replace Logo" : "Upload Logo"}
//           </button>
//         </div>
//       </div>
...
//                         {sheet.projectLogo || project?.logoUrl ? (
//                           <img src={sheet.projectLogo || project.logoUrl} alt="Project logo" style={styles.logoImageSmall} />
//                         ) : (
...
//                       <button onClick={() => setSelectedCalendar(padCalendarDays(sheet))} style={selectedCalendar?.id === sheet.id ? styles.viewBtnActive : styles.viewBtn}>
//                         {selectedCalendar?.id === sheet.id ? "Viewing" : "Open Calendar"}
//                       </button>
//                     </td>
//                   </tr>
//                 ))}
...
const SMMManagerView = ({ projectId }) => {
  ...
  return (
    <div className="smm-container" style={styles.container}>
      ...
      <div className="smm-logo-section">
        <div style={styles.logoBox} onClick={handleLogoClick}>
          {/* FIXED: Changed project?.logoUrl to project?.logo */}
          {project?.logo ? (
              ...
              <tbody>
                {monthlySheets.map((sheet) => (
                  <tr key={sheet.id} style={{ ...styles.tableRow, backgroundColor: selectedCalendar?.id === sheet.id ? "#f0fdf4" : "transparent" }}>
                    ...
                    <td style={styles.tdCell}>
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        {/* FIXED: Changed sheet.projectLogo || project?.logoUrl to sheet.projectLogo || project?.logo */}
